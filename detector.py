...
class Detection:
    def __init__(self, target_blocks: List[str], radius: int, MAX_WORKERS: int, EYE_HEIGHT: float = 1.62):
    ...
    def locate_blocks(self) -> Tuple[List[Tuple[int,int,int]], List[List[Tuple[float,float]]]]:
        ...
        def check_block(i: int) -> Optional[Tuple[Tuple[int,int,int], List[Tuple[float,float]]]]:
            if any(t in blocks[i] for t in self.target_blocks):
                angles: List[Tuple[float,float]] = raycast_block_subregions(self.origin, positions[i], rays_per_axis=3)
                if angles:
